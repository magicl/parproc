...
def define_procs() -> None:
    ...
    def build(context: pp.ProcContext, cluster_name: str) -> None:
        time.sleep(1)

    @pp.Proto(name='k8s.push::[cluster_name]', deps=['@k8s.build::[cluster_name]'])
    def push(context: pp.ProcContext, cluster_name: str) -> None:
        time.sleep(1)

    @pp.Proto(name='k8s.deploy::[cluster_name]', deps=['@k8s.push::[cluster_name]', '@cdn.upload'])
    def deploy(context: pp.ProcContext, cluster_name: str) -> None:
        time.sleep(1)
...
def main() -> None:
    ...
    pp.run('k8s.deploy::[cluster_name]', cluster_name=args.cluster_name)
