...
class RdepTest(TestCase):
    ...
    def test_rdep_execution_order(self):
        ...
        self.assertIn('setup', execution_order)
        self.assertIn('B', execution_order)
        self.assertNotIn('B_before_setup', execution_order)
        # Verify setup comes before B in the order
        setup_idx = execution_order.index('setup')
...
class RdepPatternMatchingTest(TestCase):
    ...
    def test_match_empty_placeholder(self):
        ...
        # This depends on implementation - currently [^-]+ requires at least one char
        # So empty strings won't match, which is probably correct
